...
- Recurring unsafe-input, grid-limit, and HAEO fallback notifications can be turned off with **Show plan fallback notifications** under **AI and safety**. Turning the option off dismisses existing fallback notifications; it does not change plan health, device eligibility, or fail-closed execution.
