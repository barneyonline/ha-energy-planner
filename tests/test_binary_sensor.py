...
class _AvailableStates:
    def __init__(self, entry_data: dict[str, object]) -> None:
        self._entity_ids = {
            item.strip() for value in entry_data.values() for item in str(value or "").split(",") if "." in item
        }
...
def test_binary_sensor_setup_and_entity_state(monkeypatch: object) -> None:
    ...
    asyncio.run(binary_sensor_module.async_setup_entry(SimpleNamespace(), entry, added.extend))
