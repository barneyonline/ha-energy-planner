"""Compatibility runs must retain all modules and propagate process failures."""
...
MODULES = {
    ...
    "test_repairs.py", "test_lifecycle.py", "test_services.py", "test_calendar.py", "test_manifest.py",
}


@pytest.mark.parametrize("exit_code", [0, 1, 139])
def test_combined_modules_preserve_test_and_crash_failures(tmp_path: Path, exit_code: int) -> None:
    ...
    result = subprocess.run(
        ["bash", str(ROOT / "scripts/docker-compatibility.sh"), "2026.9.0", "stable"],
    ...
    versions = ["2026.9.0", "stable"] if exit_code == 0 else ["2026.9.0"]
